...
scene.fog = new THREE.FogExp2(0x8fc8ee, 0.0065);
...
function terrainColor(height, slopeSignal) {
  if (height < -8) return new THREE.Color(0x4d7f72);
  if (height < 4) return new THREE.Color(0x6fae61).lerp(new THREE.Color(0xc2ad65), slopeSignal);
  if (height < 19) return new THREE.Color(0x4f8a4d).lerp(new THREE.Color(0x92866b), slopeSignal);
  if (height < 31) return new THREE.Color(0x7f8a78).lerp(new THREE.Color(0xc7c0a9), slopeSignal);
  return new THREE.Color(0xd9dbc9).lerp(new THREE.Color(0xffffff), Math.min(1, (height - 30) / 18));
}

function isLakeArea(x, z, padding = 0) {
  return Math.hypot(x + 74, z - 58) < 64 + padding;
}
...
function addWater() {
  ...
  const lakeGeometry = new THREE.CircleGeometry(64, 128);
  lakeGeometry.rotateX(-Math.PI / 2);
  const lake = new THREE.Mesh(lakeGeometry, waterMaterial);
  lake.scale.set(1.24, 1, 0.82);
  lake.position.set(-74, -4.8, 58);
  lake.userData.baseY = lake.position.y;
  features.add(lake);
  animatedWater.push(lake);

  for (let i = 0; i < 9; i += 1) {
    const streamGeometry = new THREE.CircleGeometry(18 - i * 0.55, 48);
    streamGeometry.rotateX(-Math.PI / 2);
    const stream = new THREE.Mesh(streamGeometry, waterMaterial);
    const x = -28 + i * 22;
    const z = 92 + Math.sin(i * 0.85) * 14;
    stream.scale.set(1.9, 1, 0.34);
    stream.rotation.y = 0.2 + Math.sin(i) * 0.32;
    stream.position.set(x, terrainHeight(x, z) + 0.18, z);
    stream.userData.baseY = stream.position.y;
    features.add(stream);
    animatedWater.push(stream);
  }
}
...
function addTrees() {
  ...
  const crownGeometries = [
    new THREE.ConeGeometry(2.25, 6.4, 8),
    new THREE.IcosahedronGeometry(2.65, 1),
    new THREE.SphereGeometry(2.4, 10, 8),
  ];
  const crownMaterials = [
    new THREE.MeshStandardMaterial({ color: 0x246b49, roughness: 0.95 }),
    new THREE.MeshStandardMaterial({ color: 0x3d8c45, roughness: 0.95 }),
    new THREE.MeshStandardMaterial({ color: 0x9b315d, roughness: 0.95 }),
  ];

  for (let i = 0; i < 280; i += 1) {
    const angle = hash(i, i + 8) * Math.PI * 2;
    const radius = 18 + hash(i + 3, i - 2) * 188;
    const x = Math.cos(angle) * radius;
    const z = Math.sin(angle) * radius;
    const y = terrainHeight(x, z);

    if (y < -5 || y > 28 || isLakeArea(x, z, 18)) continue;

    const tree = new THREE.Group();
    const trunk = new THREE.Mesh(trunkGeometry, trunkMaterial);
    const crownIndex = Math.floor(hash(i + 12, i - 4) * crownGeometries.length);
    const crown = new THREE.Mesh(crownGeometries[crownIndex], crownMaterials[crownIndex]);
    trunk.position.y = 2;
    crown.position.y = crownIndex === 0 ? 7 : 6.15;
    trunk.castShadow = true;
    crown.castShadow = true;
    tree.add(trunk, crown);
    tree.position.set(x, y, z);
    tree.rotation.y = hash(i + 40, i) * Math.PI;
    const scale = 0.58 + hash(i - 10, i + 20) * 0.86;
...
function addRocks() {
  ...
  const rockMaterials = [
    new THREE.MeshStandardMaterial({ color: 0x6f7369, roughness: 0.96 }),
    new THREE.MeshStandardMaterial({ color: 0x8c887d, roughness: 0.96 }),
  ];

  for (let i = 0; i < 96; i += 1) {
    const angle = hash(i + 500, i - 12) * Math.PI * 2;
    const radius = 24 + hash(i - 60, i + 2) * 182;
    const x = Math.cos(angle) * radius;
    const z = Math.sin(angle) * radius;
    const y = terrainHeight(x, z);

    if (y < -4 || isLakeArea(x, z, 10)) continue;

    const rock = new THREE.Mesh(rockGeometry, rockMaterials[i % rockMaterials.length]);
    rock.position.set(x, y + 0.55, z);
    rock.rotation.set(hash(i, 2) * Math.PI, hash(i, 3) * Math.PI, hash(i, 4) * Math.PI);
    rock.scale.set(
      0.8 + hash(i, 11) * 2.8,
      0.42 + hash(i, 12) * 1.4,
      0.7 + hash(i, 13) * 2.4,
...
function addMeadowDetails() {
  ...
  const grassGeometry = new THREE.ConeGeometry(0.08, 1.1, 3);
  const grassMaterial = new THREE.MeshStandardMaterial({ color: 0x7fc45f, roughness: 1 });
  const grass = new THREE.InstancedMesh(grassGeometry, grassMaterial, 1400);

  const flowerGeometry = new THREE.SphereGeometry(0.16, 6, 4);
  const flowerMaterials = [
    new THREE.MeshStandardMaterial({ color: 0xe0b857, roughness: 0.86 }),
    new THREE.MeshStandardMaterial({ color: 0xf07f74, roughness: 0.86 }),
    new THREE.MeshStandardMaterial({ color: 0x86d8c8, roughness: 0.86 }),
  ];
  const flowers = flowerMaterials.map((material) => new THREE.InstancedMesh(flowerGeometry, material, 180));
  const flowerCounts = [0, 0, 0];
  let grassCount = 0;

  for (let i = 0; i < 2200 && grassCount < 1400; i += 1) {
    const x = (hash(i + 700, i) - 0.5) * terrainSize * 0.92;
    const z = (hash(i, i + 701) - 0.5) * terrainSize * 0.92;
    const y = terrainHeight(x, z);
    const slope = Math.abs(terrainHeight(x + 1.8, z) - terrainHeight(x, z + 1.8));

    if (y < -3 || y > 18 || slope > 3.8 || isLakeArea(x, z, 8)) continue;

    dummy.position.set(x, y + 0.55, z);
    dummy.rotation.set(0, hash(i, 88) * Math.PI * 2, (hash(i, 91) - 0.5) * 0.38);
    dummy.scale.setScalar(0.55 + hash(i, 92) * 0.95);
    dummy.updateMatrix();
    grass.setMatrixAt(grassCount, dummy.matrix);
    grassCount += 1;

    if (i % 7 === 0) {
      const flowerIndex = Math.floor(hash(i, 95) * flowers.length);
      const index = flowerCounts[flowerIndex];
      if (index < 180) {
        dummy.position.set(x, y + 0.98, z);
        dummy.rotation.set(0, 0, 0);
        dummy.scale.setScalar(0.75 + hash(i, 96) * 0.8);
        dummy.updateMatrix();
        flowers[flowerIndex].setMatrixAt(index, dummy.matrix);
  ...
  flowers.forEach((flower, index) => {
    flower.count = flowerCounts[index];
...
function addCabins() {
  const wallMaterial = new THREE.MeshStandardMaterial({ color: 0x8b6042, roughness: 0.9 });
  const roofMaterial = new THREE.MeshStandardMaterial({ color: 0x5b3b35, roughness: 0.92 });
  ...
  spots.forEach(([x, z, rotation]) => {
    const y = terrainHeight(x, z);
    if (y < -2 || y > 16 || isLakeArea(x, z, 12)) return;

    const cabin = new THREE.Group();
    const body = new THREE.Mesh(new THREE.BoxGeometry(7.8, 4.8, 6.4), wallMaterial);
    const roof = new THREE.Mesh(new THREE.ConeGeometry(5.8, 3.2, 4), roofMaterial);
    const window = new THREE.Mesh(new THREE.BoxGeometry(1.4, 1.2, 0.12), lightMaterial);
    body.position.y = 2.4;
    roof.position.y = 6.35;
    roof.rotation.y = Math.PI / 4;
    window.position.set(0, 3, 3.26);
    body.castShadow = true;
    roof.castShadow = true;
    cabin.add(body, roof, window);
...
function addClouds() {
  ...
  for (let i = 0; i < 18; i += 1) {
    ...
    const parts = 4 + Math.floor(hash(i, 320) * 4);

    for (let j = 0; j < parts; j += 1) {
      const puff = new THREE.Mesh(new THREE.SphereGeometry(4.2, 10, 8), cloudMaterial);
      puff.position.set((j - parts / 2) * 4.8, hash(i, j) * 1.8, (hash(j, i) - 0.5) * 5);
      puff.scale.set(1.25 + hash(i, j + 4) * 1.4, 0.48 + hash(i, j + 5) * 0.32, 0.8 + hash(i, j + 6));
      cloud.add(puff);
    }

    cloud.position.set(
      (hash(i + 1000, i) - 0.5) * 520,
      62 + hash(i, i + 1001) * 44,
      (hash(i + 1002, i) - 0.5) * 480,
    );
    cloud.userData.speed = 1.2 + hash(i, 77) * 2.3;
    cloud.userData.wrap = 270 + hash(i, 78) * 60;
    clouds.push(cloud);
    features.add(cloud);
  }
}
...
function updateWorldDetails(delta, time) {
  animatedWater.forEach((water, index) => {
    water.position.y = water.userData.baseY + Math.sin(time * 0.0018 + index) * 0.05;
    water.material.opacity = 0.66 + Math.sin(time * 0.0014 + index * 0.7) * 0.05;
  });
